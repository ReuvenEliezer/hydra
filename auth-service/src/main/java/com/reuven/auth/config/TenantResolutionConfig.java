...
/**
 * Wires tenant resolution's configuration surface, mirroring {@link CorsConfig}: this service
 * has no {@code @ConfigurationPropertiesScan}, so a {@code @ConfigurationProperties} record that
...
public class TenantResolutionConfig {
    ...
    public TenantHostParser tenantHostParser() {
        if (tenantProperties.baseDomains().isEmpty()) {
            log.warn("hydra.tenant.base-domains is empty - every address will resolve to 'unknown' and no login can succeed");
        } else {
            log.info("Tenant resolution enabled for base domains {}", tenantProperties.baseDomains());
        }
        return new TenantHostParser(tenantProperties.baseDomains());
    }
}
